        ...
        <div className="aspect-square w-full bg-white/50 border-2 border-dashed border-gray-300 rounded-lg flex flex-col justify-center items-center hover:border-indigo-500 hover:bg-white/75 transition-colors duration-300">
          {image ? (
            ...
            <div className="text-center text-gray-500">
              ...
              <span className="mt-2 block font-medium">Upload Person Photo</span>
            </div>
          )}
        </div>
      </label>
...
function ImageGeneratorForm() {
    ...
    const handleGenerate = () => {
        if (!image1 || !image2) {
            // Using a custom modal/alert in a real app would be better than window.alert
        ...
        // Simulate an API call to generate the image
        setTimeout(() => {
            // In a real app, this would be the URL from your image generation API
            const placeholderUrl = `https://placehold.co/600x400/8B5CF6/FFFFFF?text=Generated+Image`;
    ...
    return (
        <div className="bg-gradient-to-br from-indigo-50 to-purple-100 max-w-4xl mx-auto my-12 p-6 sm:p-8 rounded-2xl shadow-xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ImageUpload image={image1} onImageChange={(e) => handleImageChange(e, setImage1)} id="file-upload-1" />
                <ImageUpload image={image2} onImageChange={(e) => handleImageChange(e, setImage2)} id="file-upload-2" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-6">
                 <div>
                    <label htmlFor="gesture" className="block text-sm font-medium text-gray-700 mb-2">Choose Gesture</label>
                    <select id="gesture" value={gesture} onChange={(e) => setGesture(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="handshake">Handshake</option>
                         {/* More options can be added later */}
                    </select>
                </div>
                 <div>
                    <label htmlFor="background" className="block text-sm font-medium text-gray-700 mb-2">Choose Background</label>
                    <select id="background" value={background} onChange={(e) => setBackground(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isLoading || generatedImage ? 'max-h-[1000px] mt-8' : 'max-h-0 mt-0'}`}>
                <div className="w-full aspect-video bg-gray-100 rounded-lg flex items-center justify-center">
                    {isLoading && <p className="text-gray-500">Processing, please wait...</p>}
